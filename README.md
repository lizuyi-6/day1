...
Aether Flow 是一个全栈 AI 应用开发平台，旨在降低 AI Agent 开发门槛，让开发者无需编写大量代码即可构建复杂的智能应用。

### 核心功能

**可视化工作流编排**
平台提供基于 Vue Flow 的拖拽式编辑器，用户可以通过拖拽节点、连接关系的方式构建 AI 工作流。支持条件分支、循环、HTTP 请求、延迟等多种节点类型，满足复杂的业务逻辑需求。

**知识库管理 (RAG)**
内置完整的文档管理和检索系统。用户可上传文本、PDF 等格式的文档，系统自动进行向量化处理（1536 维嵌入）并存储。在对话时，系统会自动检索相关文档内容，结合大语言模型生成更准确的回答。

**多模型集成**
兼容 OpenAI GPT 系列、Anthropic Claude、阿里云通义千问等多个主流 LLM 提供商。用户可根据需求切换不同的模型，也可以接入自定义的模型服务。

**会话管理**
完整的多轮对话系统，支持会话历史记录、上下文保持和流式响应输出。用户可以基于历史会话进行连续对话，系统会维护完整的对话上下文。

### 应用场景

**智能客服系统**
结合知识库和 LLM，构建能够准确回答产品问题的客服机器人。通过上传产品文档、FAQ 等资料，机器人可以给出专业的回答。

**内容生成工作流**
利用条件节点和循环节点，构建自动化的内容生成流程。例如：生成文章摘要、翻译多语言文本、批量生成产品描述等。

**数据分析助手**
连接外部数据 API，通过工作流编排实现数据获取、处理、分析和报告生成的完整流程。

**知识管理平台**
企业内部知识库管理系统，支持文档上传、智能检索、问答交互等功能。

### 技术亮点

**前后端分离架构**
前端采用 Vue 3 + TypeScript + Vite，提供流畅的用户体验；后端基于 NestJS 构建，模块化设计便于扩展维护。

**容器化部署**
完整的 Docker Compose 配置，实现 PostgreSQL 数据库、NestJS 后端、Vue 3 前端的一键部署。

**类型安全**
全栈 TypeScript 实现，从前端组件到后端 API 接口都有完整的类型定义，降低运行时错误。

**可扩展性**
基于模块的架构设计，支持自定义节点类型、集成新的 LLM 提供商、扩展数据存储方案等。
